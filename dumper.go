...
func (d *Dumper) Dump(collection *mgo.Collection, model parser.Model) error {
    err := collection.Update(model.FormKey(), model)
...
func (d *Dumper) Characters(characters ...parser.Character) error {
    for _, character := range characters {
        err := d.Dump(d.characters, character)
        if err != nil {
            return err
        }
    }
